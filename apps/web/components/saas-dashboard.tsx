...
export const SaasDashboard = () => {
  ...
  const evidenceGroups = useMemo(() => {
    const sorted = [...evidenceSnippets].sort((a, b) => ((b.credibilityScore ?? b.sourceQualityScore ?? 0) - (a.credibilityScore ?? a.sourceQualityScore ?? 0));
    const top = sorted.filter((s) => (s.credibilityScore ?? s.sourceQualityScore ?? 0) >= 85);
    const support = sorted.filter((s) => (s.credibilityScore ?? s.sourceQualityScore ?? 0) >= 60 && (s.credibilityScore ?? s.sourceQualityScore ?? 0) < 85);
    const weak = sorted.filter((s) => (s.credibilityScore ?? s.sourceQualityScore ?? 0) < 60);
