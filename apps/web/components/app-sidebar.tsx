...
const planMeta: Record<UserPlan, { label: string; limit: number; accent: string }> = {
  free: { label: "Free Beta", limit: 10, accent: "text-slate-100" },
  pro: { label: "Pro", limit: 50, accent: "text-violet-100" },
  ultra: { label: "Ultra", limit: 150, accent: "text-cyan-100" }
};
  ...
  return (
    <aside className="hidden min-h-screen w-[260px] shrink-0 border-r border-slate-800 bg-[#0b1020] p-4 lg:block">
      ...
      <div className="space-y-6">
        ...
        <div><p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-slate-500">Account</p><div className={sectionClass}><Link href="/pricing" className={itemClass}>Usage & Plan</Link>{isLoggedIn && onLogout ? <button className={itemClass} type="button" onClick={onLogout}>Logout</button> : null}</div></div>
      </div>
