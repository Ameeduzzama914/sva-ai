...
export type ModelAnswerSource = "openrouter" | "provider" | "fallback_generated";
...
export interface PerModelSource { model: ModelName; source: ModelAnswerSource; fallbackState: ModelFallbackState; providerModelId?: string; errorMessage?: string; statusCode?: number; providerErrorType?: string; }
...
export type ClaimVerificationStatus = "strongly_supported"|"supported"|"mostly_supported"|"mixed_evidence"|"misleading"|"contradicted"|"false_premise"|"disputed"|"insufficient_evidence";
export interface ClaimVerification { id: string; claim: string; category?: "benefit"|"risk"|"scientific_consensus"|"contradiction"|"safety"|"effectiveness"|"timeline"|"side_effect"|"recommendation"|"general"; status: ClaimVerificationStatus; confidenceScore: number; claimConfidenceScore?: number; supportingEvidence: EvidenceSnippet[]; linkedEvidenceIds?: string[]; evidenceRelevanceScore?: number; evidenceCredibilityScore?: number; contradictedByModels: ModelName[]; explanation: string; }
export interface VerificationResult { agreementScore:number; evidenceAlignmentScore:number; finalConfidenceScore:number; claimCoverageScore?: number; verificationBadges?: string[]; sections?: { coreConclusion: string; evidenceSummary: string; risksAndCaveats: string; contradictions: string; scientificConsensusSummary: string; }; contradictionSeverity?: "low"|"medium"|"high"; confidenceLabel:"Very High"|"High"|"Medium"|"Low"; finalAnswer:string; majorityModels:ModelName[]; outlierModels:ModelName[]; reasoning:string; explanation:string; claimVerifications:ClaimVerification[]; contradictionScore?:number; contradictionPenalty?:number; contradictionType?:"direct"|"temporal"|"consensus_shift"|"contextual"; consensusEvolutionScore?:number; consensusEvolutionSummary?:string; sourceQualityScore?:number; judgeVerdict?:"supported"|"mostly_supported"|"mixed_evidence"|"unverified"|"misleading"|"contradicted"|"false_premise_detected"|"approved"|"rejected"|"caution"|"needs_review"; judgeSummary?:string; judgeRiskFlags?:string[]; deepAnalysisNotes?:string; researchSummary?:string; trustBreakdown?:{agreementContribution:number;evidenceContribution:number;sourceContribution:number;contradictionImpact:number;}; whyNotHigher?:string; debug?:{groupScores?:Array<{model:ModelName;bestGroupScore:number;assignedGroupIndex:number}>;weightedAgreement?:{majorityWeight:number;totalWeight:number};modelSupportScore?:number;responseConsistencyScore?:number;semanticAgreement?:number;numericConsistency?:number;evidenceCoverage?:number;contradictionReasons?:ModelName[];confidenceBreakdown?:{agreement:number;evidence:number;source:number;contradiction:number};evidenceScoreBreakdown?:{credibility:number;coverage:number};normalizedResponses?:Array<{model:ModelName;answer:string;entities:string[];numbers:string[];claims:string[]}>;uncertaintyDivergence?:number;unifiedVerificationState?:{agreementScore:number;evidenceScore:number;contradictionScore:number;uncertaintyScore:number;hallucinationPenalty:number;semanticDivergence:number;sourceCredibility:number;retrievalSuccess:boolean;claimSupportLevel:number;finalConfidence:number;trustClassification:"high"|"medium"|"low";verifiedClaims:number;disputedClaims:number;unsupportedClaims:number;temporalPenalty:number;contradictionReasons:string[]};}; }
export interface VerificationExecutionMeta { mode?:"openrouter"|"pro"; modeUsed?:"fast"|"deep"|"research"; modelLayer?:"free"|"pro"; gptSource?:ModelAnswerSource; geminiSource?:ModelAnswerSource; deepseekSource?:ModelAnswerSource; modelASource?:ModelAnswerSource; modelBSource?:ModelAnswerSource; modelCSource?:ModelAnswerSource; providerMessage?:string; retrievalModeUsed?:"web"|"none"; retrievalSourceCount?:number; responseQualityFlag?:"normal"|"low_response_count"; complexity?:"simple"|"normal"|"complex"; comparisonMaxTokens?:number; synthesisMaxTokens?:number; synthesisRetryCount?:number; synthesisTruncated?:boolean; }
export interface RuntimeProviderStatus { configured:boolean; liveSuccess:boolean; source:ModelAnswerSource; fallbackState:ModelFallbackState; status?:"pending"|"success"|"failed"|"timeout"|"fallback"; errorMessage?:string; statusCode?:number; providerErrorType?:string; providerModelId?:string; actualModelId?:string; latencyMs?:number; promptTokens?:number; completionTokens?:number; reasoningTokens?:number; cachedTokens?:number; costUsd?:number; rawResponse?:string; }
export interface ProviderUsageAttempt { modelFamily:"gpt"|"gemini"|"deepseek"|"synthesis"; requestedModel:string; actualModel?:string; attemptType:"primary"|"fallback"|"synthesis"|"synthesis_retry"; promptTokens?:number; completionTokens?:number; reasoningTokens?:number; cachedTokens?:number; costUsd?:number; latencyMs?:number; statusCode?:number; errorType?:string; success:boolean; }
export interface VerificationUsageSummary { plan:"free"|"pro"|"ultra"; usedToday:number; dailyLimit:number; creditsRemaining:number; }
