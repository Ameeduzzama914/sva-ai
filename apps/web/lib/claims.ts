...
const isLikelyFactualClaim = (claim: string): boolean => {
  if (!claim || claim.length < 10) return false;
  const normalized = normalizeClaim(claim);
  if (!normalized) return false;
  if (VAGUE_PATTERNS.some((pattern) => pattern.test(claim)) || META_PATTERNS.some((pattern)=>pattern.test(claim))) return false;
...
export const extractClaims = (text: string): string[] => {
  ...
  segments.forEach((segment) => {
    ...
    if (keys.some((k) => tokenOverlap(k, thisTokens) >= 0.72)) return;
