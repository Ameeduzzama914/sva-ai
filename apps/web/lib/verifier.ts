...
export const buildResponsesForPrompt = async (
  ...
  const MODELS = [
    { id: "mistralai/mistral-7b-instruct", name: "GPT" as const },
    { id: "meta-llama/llama-3-8b-instruct", name: "Gemini" as const },
    { id: "google/gemma-7b-it", name: "DeepSeek" as const }
  ];
  ...
  const providerRuntimeStatus: Record<ModelName, RuntimeProviderStatus> = {
    "GPT": { configured: Boolean(process.env.OPENROUTER_API_KEY), liveSuccess: true, source: "openrouter", fallbackState: "none" },
    "Gemini": { configured: Boolean(process.env.OPENROUTER_API_KEY), liveSuccess: true, source: "openrouter", fallbackState: "none" },
    "DeepSeek": { configured: Boolean(process.env.OPENROUTER_API_KEY), liveSuccess: true, source: "openrouter", fallbackState: "none" }
  };
