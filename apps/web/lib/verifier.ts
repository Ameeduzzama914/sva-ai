...
export const verifyResponses = (
  ...
  const contradictionSeverityLabel = contradictionSeverity(contradiction.contradictionScore);
  ...
  const judge = buildJudgeAssessment({
    ...
    verificationBadges: [agreementScore >= 75 ? "Strong Consensus" : "Scientific Debate", evidenceAlignmentScore < 45 ? "Weak Evidence" : "Evidence-backed", adjustedFinalConfidence >= 75 ? "High Confidence" : "Low Confidence"],
    sections,
    contradictionSeverity: contradictionSeverityLabel,
    evidenceAlignmentScore,
