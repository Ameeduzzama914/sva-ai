...
const buildJudgeAssessment = (input: {
  ...
  const weakClaims = input.claimVerifications.filter((claim) =>
    ["weak_support", "contradicted", "insufficient_evidence", "unsupported", "uncertain"].includes(claim.status)
  ).length;
...
export const verifyResponses = (
  ...
  const unified = unifiedTrustScore({
  ...
  adjustedFinalConfidence = unified.score;
  ...
  return {
    ...
    trustBreakdown: {
      ...
      contradictionImpact: Math.round(unified.contradictionImpact)
    },
    whyNotHigher:
      confidence.score >= 90
