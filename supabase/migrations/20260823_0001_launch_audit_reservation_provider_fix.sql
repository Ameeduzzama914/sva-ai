-- Launch audit hardening: make reservation idempotency truly no-op on duplicate keys,
-- allow valid cancellation-at-period-end access, and align provider_usage constraints
-- with synthesis retry accounting. Safe to rerun. Does not delete production data.
...
alter table public.subscriptions
  ...
  check (status in ('inactive', 'active', 'past_due', 'halted', 'cancelled', 'expired', 'cancel_at_period_end'));

alter table public.provider_usage
  drop constraint if exists provider_usage_attempt_type_check;

alter table public.provider_usage
  add constraint provider_usage_attempt_type_check
  check (attempt_type in ('primary', 'fallback', 'synthesis', 'synthesis_retry', 'evidence'));
